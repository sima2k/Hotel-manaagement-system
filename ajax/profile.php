<?php 
    ...
    if(isset($_POST['info_form']))
    {
        ...
        $values = [$frm_data['name'],$frm_data['address'],$frm_data['phonenum'],$frm_data['pincode'],$frm_data['dob'],$_SESSION['id']];

        if(update($query,$values,'ssssss')){
            $_SESSION['name'] = $frm_data['name'];
            echo 1;
        }
        else{
            echo 0;
    ...
    if(isset($_POST['profile_form']))
    {
        ...
        $u_exist = select("SELECT `profile` FROM `user_cred` WHERE `id`=? LIMIT 1",[$_SESSION['id']],"s");
